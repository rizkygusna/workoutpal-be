...
router.get('/', verifyToken, async (req, res) => {
  ...
  try {
    ...
    const result = await client.execute(`SELECT * FROM exercise WHERE user_id IS NULL OR user_id = '${userId}'`);
...
router.get('/:exerciseId', verifyToken, async (req, res) => {
  const exerciseId = req.params.exerciseId;
  try {
    const result = await client.execute(`SELECT * FROM exercise WHERE exercise_id = '${exerciseId}'`);
...
router.delete('/:exerciseId', verifyToken, async (req, res) => {
  const exerciseId = req.params.exerciseId;
  try {
    const result = await client.execute(`DELETE FROM exercise WHERE exercise_id = ${exerciseId}`);
    if (result.rowsAffected <= 0) return res.status(404).json('Exercise not found.');
    res.status(204).end();
  } catch (error) {
    console.log(error);
